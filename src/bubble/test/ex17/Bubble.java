...
public class Bubble extends JLabel implements Moveable {
	...
	public void attack() {
		...
		enemy.setState(1);
		setIcon(bubbled);
		mContext.remove(enemy); // 메모리에서 사라지게 한다. (가비지 컬렉션 - > 즉시 발동하지 않음.) 즉시 사라지지 않는다는 말.
	...
	private void clearBubble() {
		try {
			...
			setIcon(bomb);
			Thread.sleep(500);
			mContext.remove(this); // BubbleFrame의 메모리에서 소멸된다.
			mContext.repaint();// BubbleFrame의 전체를 다시 그린다. (메모리에 없는건 그리지 않음)
		} catch (InterruptedException e) {
			e.printStackTrace();
		}
